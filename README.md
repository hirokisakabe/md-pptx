...
Marp 互換の Markdown からテンプレート PPTX を使って編集可能な PowerPoint ファイルを生成するツールです。

Marp で書いた Markdown を最小限の修正で移行でき、テンプレートのレイアウトやプレースホルダを活かした PPTX を出力します。

## 特徴

- **Marp 互換の Markdown 記法** — `---` スライド区切り、HTML コメントディレクティブ、画像サイズ指定などをサポート
...
Marp 互換のサイズ指定構文をサポートしています（解析のみ。出力サイズへの反映は未実装）。
