...
describe("toPlaceholderType", () => {
  it("TITLE (1) を title に変換する", () => {
    expect(toPlaceholderType(1)).toBe("title");
  });

  it("CENTER_TITLE (3) を title に変換する", () => {
    expect(toPlaceholderType(3)).toBe("title");
  });
  ...
  it("FOOTER (15) を other に変換する", () => {
    expect(toPlaceholderType(15)).toBe("other");
  });
